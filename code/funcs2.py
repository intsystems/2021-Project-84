...
def distill(out, batch_logits, temp):    
    t1 = out
    t2 = batch_logits
    ind = list(range(batch_logits.shape[0]))[::-1]
    t3 = batch_logits[ind]
    pd = 1.0 - cka(t1, t2)
    nd = 1.0 - cka(t1, t3)
    return torch.clamp(pd - nd + temp, min=0.0)
    
    return torch.nn.functional.triplet_margin_with_distance_loss(t1,t2,t3, distance_function = lambda x,y: 1.0-cka(x,y), margin = temp)
...
def param_loss(batch,model,h):
    ...
    lambda1,temp = h
    lambda2 = 1.0 - lambda1
    out0 = model.get_features(x, layers = [3])[0]
    out = model(x)    
    distillation_loss = distill(out0, batch_logits, temp)
...
def cifar_with_validation_set(exp_ver, run_num, epoch_num, filename, tr_s_epoch, m_e, tr_load, t_load, val_load, validate_every_epoch,  class_num=10, lambdas = None,  lr0 = 1e-3, lr = 1.0, clip_grad = 10e-3, mode='opt', no_tqdm = False, seed=42, logits = '../code/logits_resnet2.npy', student_class = Cifar_Very_Tiny):
    np.random.seed(seed)
    t.manual_seed(seed)
    ...
    for _ in range(run_num):
        ...
        lambda1 = t.nn.Parameter(t.tensor(np.random.uniform(low=0.0, high=1.0), device=device), requires_grad=True)
        #lambda2 = t.nn.Parameter(t.tensor(np.random.uniform(low=0.0, high=1.0), device=device), requires_grad=True)
        temp = t.nn.Parameter(t.tensor(np.random.uniform(low=0.0, high=1.0), device=device), requires_grad=True)
...
def cifar_with_hyperopt(exp_ver, run_num, epoch_num, filename, tr_s_epoch, m_e, tr_load, t_load, val_load, validate_every_epoch, trial_num, lr0=1.0, logits = '../code/logits_cnn.npy', student_class = Cifar_Very_Tiny):
    ...
    for _ in range(run_num):
        cost_function = lambda lambdas: -cifar_with_validation_set(exp_ver, 1, epoch_num, None, tr_s_epoch, m_e, tr_load, t_load, val_load, validate_every_epoch, lambdas = [lambdas[0],lambdas[1]],  mode='no-opt',lr0=lr0, logits=logits, student_class=student_class) # validation accuracy * (-1) -> min
       
        best_lambdas = fmin(fn=cost_function,                             
        #space= [ hp.uniform('lambda1', 0.0, 1.0), hp.uniform('lambda2', 0.0, 1.0), hp.uniform('temp', 0.1, 10.0)],
        space= [ hp.uniform('lambda1', 0.0, 1.0), hp.uniform('temp', 0.0, 1.0)],  
        algo=tpe.suggest,
        max_evals=trial_num)
